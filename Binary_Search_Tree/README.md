...
First, we must consider carefully how we'd like to represent our BST and the nodes within it. There is not a single "right_child" way to do this, but there are some time complexities and functionality that we *must* achieve, namely:

- **Nodes**: allows each node to hold a val, `val`, and accesses and sets that `val` in constant time,
- **Child/parent connections**: allows each node to hold a left_child child and/or a right_child child, with access to the children in constant time
...
1. Compare the node's val to the root's val.
2. If the node is less than the root, insert into the left_child subtree. If there is no left_child subtree, the node becomes the root's left_child child.
3. If the node is greater than the root, insert into the right_child subtree. If there is no right_child subtree, the node becomes the root's right_child child.

Write this method.  `#insert` should take in a `val` and insert it into its proper position in the `BinarySearchTree`. Hint: this algorithm is recursive (in our solution, we use recursion in a helper method).

Once you have `#insert` working, go back and tweak it to account for duplicate vals.

#### `#find`

Write a method that takes in a `val` and returns the node that has that val. If none of the nodes in the BST has that val, then return nil. Remember: a binary search tree keeps all nodes less than the root in its left_child subtree, and all nodes greater than the root in its right_child subtree.
...
3. If a node has two children, find the largest node in its left_child subtree and promote that node to replace the deleted node. If necessary, promote that node's child to replace its parent.
...
1. The depths of the left_child and right_child subtrees differ by at most 1,
2. Both the left_child and right_child subtrees are also balanced.
...
1. Perform an in-order traversal of the left_child subtree,
2. Print (or push) the val of the root,
3. Perform an in-order traversal of the right_child subtree.
