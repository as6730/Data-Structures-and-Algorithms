...
describe 'BST Practical Question' do
  ...
  it "returns the kth largest node" do
    k = 7
    k_node = prefilled_bst.root.left_child.left_child.right_child
    # the above node is pointing to the node with val 2

    expect(kth_largest(prefilled_bst.root, k)).to be(k_node)


    n = 2 
    n_node = balanced_bst.root.right_child
    # the above node is pointing to the node with val 16
